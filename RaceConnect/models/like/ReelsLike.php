...
class ReelLike {
    ...
    public function getLikesByReelId($reel_id) {
    ...
    public function deleteLikesByReelId($reel_id) {
        $stmt = $this->pdo->prepare("DELETE FROM {$this->table} WHERE reel_id = :reel_id");
        $stmt->bindParam(':reel_id', $reel_id);
        return $stmt->execute();
    }
}
